...
unsigned char *read_png(char *file_name) {
    ...
    if (!fp) {
        abort_("[read_png] File %s could not be opened for reading",
               file_name);
    }
    fread(header, 1, 8, fp);
    if (png_sig_cmp(header, 0, 8)) {
        abort_("[read_png] Not a PNG file");
    }
...
int main(int argc, char **argv) {
    ...
    if (argc == 10 && strcmp(argv[1], "-p") == 0 &&
        ...
        int img_side = ceil(sqrt(fsize + HEADER_LENGTH + FILE_LENGTH));
        width = img_side;
        height = img_side;
        ...
        size_t nread =
            fread(&img_data[HEADER_LENGTH + FILE_LENGTH], fsize, 1, infile);
        ...
        unsigned char *img_data = read_png(argv[6]);
